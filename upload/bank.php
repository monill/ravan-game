...
function withdraw()
{
    ...
    if ($ir['bankmoney'] < $_POST['wd']) {
        echo "You Tried Withdrawing <b>\$$with</b> but you only have <b>\$$bkmon</b></div><div><img src='images/generalinfo_btm.jpg' alt='' /></div><br></div></div></div></div></div></a>";
        $h->endpage();
        exit;
    }
...
function deposit()
{
    ...
    if ($ir['money'] < $_POST['dt']) {
        echo "You tried depositing <b>\$$dep</b> but you only have <b>\$$mon</b><br> <a href='bank.php'>Back</a></div><div> <img src='images/generalinfo_btm.jpg' alt='' /> </div><br></div></div></div></div></div>";
        $h->endpage();
        exit;
    }
    $blah = $_POST['dt'] / 50;
